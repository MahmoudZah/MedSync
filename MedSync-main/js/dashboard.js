...
function calculateDashboardStats(pharmacyId) {
  ...
  pharmacyOrders.forEach(order => {
    order.items.forEach(item => {
      ...
      if (listing) {
        wasteSaved += (listing.originalPrice - listing.discountPrice) * item.quantity;
      }
    });
  });

  // Get expiring soon batches (< 30 days)
  const listings = db.listings.filter(l => l.pharmacyId === pharmacyId && l.quantity > 0);
...
function renderDashboard() {
  ...
  // Render pharmacy/company name
  // Look in both companies and pharmacies for backwards compatibility
  let company = db.companies?.find((p) => p.id === (user.companyId || user.pharmacyId));
  if (!company && db.pharmacies) {
    company = db.pharmacies.find((p) => p.id === (user.companyId || user.pharmacyId));
  }
  if (company) {
    document.getElementById("pharmacyName").innerText = company.name;
  }

  // Calculate and render stats
  const userCompanyId = user.companyId || user.pharmacyId;
  const stats = calculateDashboardStats(userCompanyId);
  document.getElementById("totalSales").innerText = `${stats.totalSales.toLocaleString()} EGP`;
  document.getElementById("potentialWasteSaved").innerText = `${stats.wasteSaved.toLocaleString()} EGP`;
  document.getElementById("expiringSoonBatches").innerText = `${stats.expiringSoon} Batches`;

  // Render inventory table
  renderInventoryTable(userCompanyId);
}
...
function renderInventoryTable(pharmacyId) {
  ...
  tbody.innerHTML = listings
    .map((listing) => {
      ...
      const statusBadge = listing.quantity <= 0
      ...
      return `
        <tr class="${listing.quantity <= 0 ? 'table-secondary' : ''}">
          ...
          <td>${statusBadge}</td>
          <td>
            <span class="${listing.quantity <= 10 ? 'text-warning fw-bold' : ''}">${listing.quantity}</span>
          </td>
          <td>
            <span class="text-decoration-line-through text-muted me-2">${listing.originalPrice}</span>
            <span class="text-success fw-bold">${listing.discountPrice} EGP</span>
          </td>
          <td class="text-end pe-4">
            <button class="btn btn-sm btn-light" onclick="editListing(${listing.id})" title="Edit">
              <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-sm btn-light text-danger" onclick="deleteListing(${listing.id})" title="Delete">
...
function renderPagination(totalPages, totalItems) {
  ...
  if (!paginationContainer) {
    ...
    paginationContainer.className = 'card-footer bg-white d-flex justify-content-between align-items-center py-3';
...
function changePage(page) {
  ...
  const totalItems = db.listings.filter((l) => l.pharmacyId === userCompanyId).length;
...
function renderOrders() {
  ...
  const orders = db.orders.filter(o => o.pharmacyId === userCompanyId);
  ...
  ordersContainer.innerHTML = orders.map(order => {
    ...
    return `
      <tr>
        <td class="ps-4"><code>#ORD-${String(order.id).padStart(4, '0')}</code></td>
...
function handleAddBatch() {
  ...
  if (!userStr) return;
  const user = JSON.parse(userStr);
  const pharmacyId = user.companyId || user.pharmacyId;
  ...
  const newListing = {
    id: db.listings.length > 0 ? Math.max(...db.listings.map((l) => l.id)) + 1 : 1,
...
function handleEditBatch() {
  ...
  listing.originalPrice = parseFloat(document.getElementById("editOriginalPrice").value);
  listing.discountPrice = parseFloat(document.getElementById("editDiscountPrice").value);
