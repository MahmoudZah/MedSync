...
function renderListings(containerId, limit = null, listingsData = db.listings) {
  ...
  container.innerHTML = listings
    .map((listing) => {
      ...
      // Skip this listing if drug or company not found
      if (!drug || !company) {
        console.log('Missing drug or company for listing:', listing);
        return '';
      }
      ...
      return `
                ...
                <div class="card h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <span class="expiry-badge ${expiryStatus.class}">${expiryStatus.text
        }</span>
                            <small class="text-muted">${listing.quantity
        } left</small>
                        </div>
          ...
          listing.discountPrice
                            ...
                            <button class="btn btn-sm btn-outline-primary" onclick="openReserveModal(${listing.id
        })">Reserve</button>
                        </div>
                    </div>
                </div>
...
function applyFilters() {
  ...
  const filtered = db.listings.filter((listing) => {
    ...
    // Skip if drug or company not found
    if (!drug || !company) return false;

    // Location Filter
    if (location && !company.location?.includes(location)) return false;
